...
from core.editor import make_arrow_cursor
...
class _ThemeRow(QWidget):
    ...
    def __init__(self, name, is_active, acc, fg, hov, is_dark=True, parent=None):
        super().__init__(parent)
        self._hov_color = hov
        self._hovered   = False
        self.setCursor(make_arrow_cursor(acc, is_dark))
        ...
        for section, names in [("Dark", dark_names), ("Light", light_names)]:
            ...
            for name in names:
                row = _ThemeRow(name, name == current, acc, fg1, hov,
                               is_dark=THEMES[name].get("is_dark", True))
