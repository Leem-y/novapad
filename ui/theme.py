...
Gradient themes removed (by design).
"""
...
THEMES = {
    ...
    "Noir":_t(
        ...
        "#111114",  # bg_toolbar
        "#0E0E11",  # bg_tab_bar
        "#0B0B0D",  # bg_tab_active
        "#111114",  # bg_tab_inactive
        "#0B0B0D",  # bg_editor
        "#0A0A0C",  # bg_statusbar
        "#111114",  # bg_menu
        "#19191E",  # bg_hover
        ...
        "#141418",  # bg_input
        "#141418",  # bg_button
        "#C8C8CF",  # accent (subtle monochrome)
        "#E6E6EA",  # accent_hover
        "#EDEDF1",  # fg_primary
        "#B8B8C0",  # fg_secondary
        "#7C7C86",  # fg_muted
        "#EDEDF1",  # fg_tab_active
        "#A8A8B2",  # fg_tab_inactive
        "#1F1F26",  # border
        "#2A2A33",  # border_input
        "#0F0F12",  # scrollbar_bg
        "#2A2A33",  # scrollbar_handle
        "#1A1A20",  # separator
        "#E8E8EE",  # sel_bg (will be overridden to very light in apply())
        "#000000",  # sel_fg
        "#2A2A33",  # find_match_bg
        "#EDEDF1",  # find_match_fg
        "#C8C8CF",  # find_cur_bg
        "#000000",  # find_cur_fg
        "#C8C8CF",  # cursor
        "#0E0E11",  # gutter_bg
...
# Keep theme lists stable and easy to scan.
THEME_NAMES = sorted(THEMES.keys(), key=lambda s: s.lower())
...
_GRADIENT_QSS: dict[str, str] = {}


def _hex_to_colorref(hex_color: str) -> int | None:
    """#RRGGBB to Windows COLORREF (0x00BBGGRR)."""
...
def apply_titlebar_color(
    ...
    *,
    ...
    """
    Match native Windows 11 caption/border to NovaPad chrome (DWM).
    Safe no-op on non-Windows, invalid HWND, or API failure.
    """
    import sys
    ...
    try:
        ...
        DwmSetWindowAttribute.restype = wintypes.BOOL

        def _set_attr(attr: int, ptr: ctypes.c_void_p, cb: int) -> None:
            DwmSetWindowAttribute(wintypes.HWND(hwnd), wintypes.DWORD(attr), ptr, wintypes.DWORD(cb))
        ...
        # Immersive dark mode (Win10 1809+); helps OS chrome match theme family.
        if build >= 17763:
            try:
                dark = ctypes.c_int(1 if is_dark else 0)
                _set_attr(
                    DWMWA_USE_IMMERSIVE_DARK_MODE,
                    ctypes.cast(ctypes.byref(dark), ctypes.c_void_p),
                    ctypes.sizeof(dark),
                )
            except Exception:
                pass
        ...
        fg_ref = _hex_to_colorref(fg_hex) if fg_hex else None
        if fg_ref is None:
            # Readable title on caption background: pick light/dark by luma of bg.
            r = int(bg_hex[1:3], 16)
            g = int(bg_hex[3:5], 16)
            b = int(bg_hex[5:7], 16)
            luma = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255.0
            fg_ref = 0x00F8F8F8 if luma < 0.45 else 0x00202020

        if build >= 22000:
            cap = ctypes.c_uint32(bg_ref)
            brd = ctypes.c_uint32(bg_ref)
            txt = ctypes.c_uint32(fg_ref)
            try:
                _set_attr(
                    DWMWA_CAPTION_COLOR,
                    ctypes.cast(ctypes.byref(cap), ctypes.c_void_p),
                    ctypes.sizeof(cap),
                )
                _set_attr(
                    DWMWA_BORDER_COLOR,
                    ctypes.cast(ctypes.byref(brd), ctypes.c_void_p),
                    ctypes.sizeof(brd),
                )
                _set_attr(
                    DWMWA_TEXT_COLOR,
                    ctypes.cast(ctypes.byref(txt), ctypes.c_void_p),
                    ctypes.sizeof(txt),
                )
            except Exception:
                pass
...
class ThemeManager:
    ...
    def apply(cls, app: QApplication, name: str):
        ...
        cls._name = name
        t = THEMES[name]
        # Force high-contrast selection:
        # - dark themes: very light selection background
        # - light themes: very dark selection background
        #
        # This makes selections obvious across all widgets, and helps OneDrive/
        # cross-device workflows where selection visibility matters.
        is_dark = bool(t.get("is_dark", True))
        acc = QColor(t["accent"])
        base_bg = QColor(t["bg_editor"])
        if is_dark:
            # Mix accent toward white, so selection is a light wash.
            sel_bg = QColor(
                int(acc.red()   * 0.25 + 255 * 0.75),
                int(acc.green() * 0.25 + 255 * 0.75),
                int(acc.blue()  * 0.25 + 255 * 0.75),
            )
            sel_fg = QColor("#000000")
        else:
            # Mix accent toward black, so selection is a dark wash.
            sel_bg = QColor(
                int(acc.red()   * 0.25),
                int(acc.green() * 0.25),
                int(acc.blue()  * 0.25),
        ...
        # Ensure selection background isn't too close to editor background.
        # If it is, fall back to pure white/black.
        if is_dark and abs(sel_bg.lightness() - base_bg.lightness()) < 40:
            sel_bg = QColor("#FFFFFF")
        if (not is_dark) and abs(sel_bg.lightness() - base_bg.lightness()) < 40:
            sel_bg = QColor("#000000")
        ...
        t_with_contrast = {**t, "sel_bg": sel_bg.name(), "sel_fg": sel_fg.name()}
        qss = _QSS.format(**t_with_contrast)
        # Gradient themes were removed; keep hook as a no-op.
        if name in _GRADIENT_QSS and _GRADIENT_QSS[name]:
            qss += _GRADIENT_QSS[name]
        app.setStyleSheet(qss)
        cls._set_palette(app, {**t, "sel_bg": sel_bg.name(), "sel_fg": sel_fg.name()})
    ...
    def _set_palette(app, t):
        ...
        p.setColor(QPalette.ColorRole.Highlight,       QColor(t["sel_bg"]))
