...
class GotoLineBar(QWidget):
    ...
    def _flash_line(self, block):
        """Highlight the target line for 600 ms then clear."""
        self._clear_flash()
        fmt = QTextCharFormat()
        fmt.setBackground(QColor("#0A84FF40"))   # semi-transparent accent
        fmt.setProperty(0x100000 + 8, True)      # FullWidthSelection
        ...
        sel.format.setProperty(
            QTextEdit.ExtraSelection.__class__,
            True
        )

        # Use ExtraSelection properly
        from PyQt6.QtGui import QTextFormat
        sel2           = QTextEdit.ExtraSelection()
        sel2.format.setBackground(QColor("#0A84FF40"))
        sel2.format.setProperty(QTextFormat.Property.FullWidthSelection, True)
        sel2.cursor    = QTextCursor(block)
        sel2.cursor.clearSelection()

        self._flash_selections = [sel2]
