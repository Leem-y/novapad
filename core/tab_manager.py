...
class NovaPadTabBar(QTabBar):
    ...
    _CLOSE_MARGIN = 6
    _PLUS_W       = 32
    _TAB_MIN_W    = 52    # never shrink below this
    _TAB_MAX_W    = 200   # never grow beyond this

    def __init__(self, parent: QWidget | None = None):
        super().__init__(parent)
        self.setMouseTracking(True)
        # Show scroll arrows when tabs are at minimum width and still overflow
    ...
    def _display_tab_rect(self, index: int) -> QRect:
        """
        Return the tab rect as currently drawn (includes any animated x-offset).
        Paint uses translated rects; hit-testing must match that.
        """
        tr = self.tabRect(index)
        dx = int(self._tab_offsets.get(index, 0.0))
        return tr.translated(dx, 0) if dx else tr

    def _tab_content_inset_top(self, index: int) -> int:
        """
        Our custom paint uses different vertical insets for active vs inactive tabs.
        Keep hit-testing/drawing (×) aligned to that painted geometry.
        """
        return 2 if index == self.currentIndex() else 4

    def _tab_content_height(self, index: int) -> int:
        inset = self._tab_content_inset_top(index)
        if index == self.currentIndex():
            # Active tab paints nearly full-height.
            return max(1, self.height() - inset)
        # Inactive tabs are shorter (top+bottom inset).
        return max(1, self.height() - inset * 2)

    def _close_rect(self, index: int, tr: QRect | None = None) -> QRect:
        tr = tr or self._display_tab_rect(index)
        sz = self._CLOSE_SIZE
        x  = tr.right() - sz - self._CLOSE_MARGIN
        # Center the close button inside the same vertical band we paint tabs in.
        # Active: top inset only (2px). Inactive: top+bottom inset (4px each).
        if index == self.currentIndex():
            top = tr.top() + 2
            ht  = max(1, self.height() - 2)
        else:
            top = tr.top() + 4
            ht  = max(1, self.height() - 8)
        y = top + (ht - sz) // 2
        return QRect(x, y, sz, sz)

    def _plus_rect(self) -> QRect:
        if self.count():
            last_right = self.tabRect(self.count() - 1).right() + 2
            # Cap so the button never drifts off-screen (scroll mode)
            x = min(last_right, self.width() - self._PLUS_W - 1)
        else:
            x = 0
    ...
    def tabSizeHint(self, index: int) -> QSize:
        ...
        # Ideal width: label + close button + padding, capped at max
        ideal = min(sh.width() + self._CLOSE_SIZE + self._CLOSE_MARGIN + 4,
                    self._TAB_MAX_W)
        # Share available bar width equally, but never below minimum
        n     = max(1, self.count())
        avail = max(self._TAB_MIN_W, self.width() - self._PLUS_W - 4)
        w     = max(self._TAB_MIN_W, min(ideal, avail // n))
        return QSize(w, sh.height())

    def minimumTabSizeHint(self, index: int) -> QSize:
        sh = super().minimumTabSizeHint(index)
        return QSize(self._TAB_MIN_W, sh.height())

    def resizeEvent(self, event):
        """Force tab widths to recompute whenever the bar changes size."""
    ...
    def paintEvent(self, event):
        ...
        for i in draw_order:
            ...
            close_reserve = self._CLOSE_SIZE + self._CLOSE_MARGIN + 4
            text_rect = tr.adjusted(8, 0, -close_reserve, 0)
            raw_label = self.tabText(i)
            fm        = painter.fontMetrics()
            label     = fm.elidedText(raw_label, Qt.TextElideMode.ElideRight,
                                      max(1, text_rect.width()))
            align = (Qt.AlignmentFlag.AlignVCenter |
                     (Qt.AlignmentFlag.AlignHCenter
                      if fm.horizontalAdvance(raw_label) <= text_rect.width()
                      else Qt.AlignmentFlag.AlignLeft))
            painter.drawText(text_rect, align, label)

            # Close button × (skip if tab is being renamed)
            if i != self._rename_index:
                cr = self._close_rect(i, tr=tr)
        ...
        # ── "+" button ────────────────────────────────────────────────────
        pr = self._plus_rect()
        # Draw a small centered button so the glyph is visually aligned.
        btn_rect = pr.adjusted(7, 7, -7, -7)
    ...
    def _animate_shifts(self):
        ...
        for i in list(self._tab_targets.keys()):
            ...
            if abs(diff) < THRESHOLD:
                ...
                elif i in self._fast_anims:
                    # Open animation complete — clean up state
                    self._fast_anims.discard(i)
                    self._tab_offsets.pop(i, None)
                    self._tab_targets.pop(i, None)
                else:
                    # Drag-reorder animation: keep offsets pinned while dragging.
                    # Otherwise tabs would "snap back" once they reach target.
                    if not getattr(self, "_drag_active", False) and float(target) == 0.0:
                        self._tab_offsets.pop(i, None)
                        self._tab_targets.pop(i, None)
            else:
...
class TabManager(QTabWidget):
    ...
    request_save         = pyqtSignal(object)  # CodeEditor
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self.setStyleSheet("QTabWidget::pane { border: none; }")
        ...
        # Save request plumbing: MainWindow (or owner) assigns this handler.
        self._save_handler = None

    def set_save_handler(self, handler) -> None:
        """
        Set a callback invoked when a modified tab needs saving.
        Signature: handler(editor: CodeEditor) -> bool
        """
    ...
    def new_tab(self, file_path: str | None = None,
        ...
        editor.document().modificationChanged.connect(
            # Don't capture the original index: tab indices change when reordered.
            lambda modified, e=editor: self._on_mod_changed(self.index_of_editor(e), modified)
        )
    ...
    def _request_save(self, editor: CodeEditor) -> bool:
        if callable(self._save_handler):
            try:
                return bool(self._save_handler(editor))
            except Exception:
                return False
        # Fallback: emit a signal so external owners can respond if they prefer signals.
        self.request_save.emit(editor)
        return False
    ...
    def remove_default_empty_tab_if_unused(self) -> bool:
        """
        Remove the first tab if it is an untouched default tab and there is at
        least one other tab. Returns True if a tab was removed.
        """
        if self.count() <= 1:
            return False
        e0 = self.editor_at(0)
        if e0 is not None and getattr(e0, "is_default_empty", None) and e0.is_default_empty():
            self.removeTab(0)
            return True
        return False
